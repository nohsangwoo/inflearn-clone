...
export default function HomePageWrapper() {
  ...
  const { data: list } = useQuery({
    ...
    queryFn: async () => {
      const apiCategory = category === "프론트엔드" ? "frontend" : category === "백엔드" ? "backend" : category === "AI" ? "ai" : undefined
  ...
  const items: CourseItem[] = (list?.items ?? []).map((c) => {
    ...
    return {
      ...
      price: effectivePrice === 0 ? "무료" : `₩${effectivePrice.toLocaleString()}`,
      originalPrice: hasDiscount ? `₩${c.price.toLocaleString()}` : undefined,
    ...
    <div className="space-y-8">
        ...
        <div className="mx-auto max-w-6xl px-4">
          <div className="flex flex-col gap-3">
            <div className="flex gap-2">
              <Input value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder="강의, 강사, 키워드를 검색하세요" />
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              {(["프론트엔드","백엔드","AI"]).map((t) => (
                <Button
                  key={t}
                  variant={category === t ? "secondary" : "outline"}
                  size="sm"
              ...
              <div className="ml-auto">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuSeparator />
                    <DropdownMenuItem onClick={() => setSort("latest")}>최신순</DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setSort("best")}>베스트</DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setSort("priceAsc")}>가격 낮은순</DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setSort("priceDesc")}>가격 높은순</DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
              </div>
            </div>
      ...
      <section>
        <div className="mx-auto max-w-6xl px-4">
          ...
          {items.length === 0 && (
            <div className="text-sm text-muted-foreground text-center py-8">검색 결과가 없습니다</div>
          )}
        </div>
      </section>
